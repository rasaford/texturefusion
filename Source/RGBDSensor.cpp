...
void RGBDSensor::saveRecordedFramesToFile( const std::string& filename )
{
	...
	if (m_bUseModernSensFilesForRecording) {
		...
		std::string actualFilename2 = filename + ".txt";
		while (util::fileExists(actualFilename)) {
			...
			actualFilename2 = path + base + std::to_string(num + 1) + ".txt";
		}
		
		m_recordedData->saveToFile(actualFilename);
		std::cout << "DONE!" << std::endl;	

		std::ofstream out(actualFilename2);
		int dFrame = -1;
		int keyFrame = 0;
		ml::mat4f previous_mat;
		ml::mat4f warp_mat;
		previous_mat.setIdentity();
		warp_mat.setIdentity();
		for (auto& a : m_recordedTrajectory) {
			if (dFrame < 0) {
				dFrame++;
				continue;
			}
